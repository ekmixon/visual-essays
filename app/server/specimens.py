...
def _get_manifest_iiifhosting(specimen, wdid, taxon_name):
    if 'manifest' not in specimen:
        ...
        if taxon_name:
            data['Taxon name'] = taxon_name
        if wdid:
            data['Wikidata entity'] = wdid.replace('wd:', 'https://www.wikidata.org/entity/')
        if 'id' in specimen and specimen['id'].startswith('jwd:'):
            data['JSTOR entity'] = specimen['id'].replace('jwd:', 'https://kg.jstor.org/entity/')
        if 'jstorPlantsId' in specimen:
            data['Global Plants ID'] = specimen['jstorPlantsId']
        if 'description' in specimen:
            data['Description'] = specimen['description']
            data['Label'] = specimen['description']
        if 'specimenType' in specimen:
            data['Specimen type'] = specimen['specimenType']
        if 'collector' in specimen:
            data['Collector'] = '; '.join(specimen['collector'])
        if 'locationCollected' in specimen:
            data['Location collected'] = specimen['locationCollected']['label']
        if 'collectionDate' in specimen:
            data['Date collected'] = specimen['collectionDate']
...
_manifests_cache = {}
def _get_manifests(data, preload=False):
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for specimen in data['specimens']:
            ...
            futures[executor.submit(_get_manifest_iiifhosting, specimen, data.get('id'), data.get('taxonName'))] = image_url
...
def get_specimens(taxon_name=None, gpid=None, wdid=None, preload=False, **kwargs):
    logger.info(f'get_specimens: taxon_name={taxon_name} gpid={gpid} wdid={wdid} max={kwargs.get("max")} preload={preload} args={kwargs}')
    if taxon_name:
    ...
    for _ in range(2):
        ...
        if resp.status_code == 200:
            ...
            logger.debug(json.dumps(data['specimens'], indent=2))
            data['specimens'] = [specimen for specimen in data['specimens'] if 'id' in specimen]
            for specimen in data['specimens']:
                data['taxonName'] = specimen.pop('taxonName', None)
                specimen.pop('@type', None)
