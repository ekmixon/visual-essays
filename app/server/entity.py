...
class KnowledgeGraph(object):
    ...
    def entity(self, uri, project=None, raw=False, article=None, **kwargs):
        ...
        if uri.startswith('http://kg.jstor.org/'):
            ...
            logger.info(json.dumps(primary, indent=2))
            # return primary
            wd_id = None
            statements = []
    ...
    def _entity_from_url(self, uri):
        uri = f'http://localhost/assets/entity/{uri.split("/entity/")[-1]}' if self.site == 'localhost' else uri
        for suffix in ('.yml', '.yaml', '.json', 'jsonld', ''):
            try:
                resp = requests.get(f'{uri}{suffix}')
                logger.info(f'{uri}{suffix} {resp.status_code}')
                if resp.status_code == 200:
                    content = resp.content.decode('utf-8')
                    if suffix in ('.yml', '.yaml'):
                        entity = yaml.load(content)
                    else:
                        entity = json.loads(content)
                    if 'id' not in entity:
                        entity['id'] = uri.replace('https', 'http').replace('.json', '').replace('.jsonld', '')
                    #if 'label' in entity:
                    #    entity = self._expand(entity)
                    return entity
            except:
                raise
        return {}

    def _expand(self, simplified):
        lang = simplified.get('language', 'en')
        entity = {
            'id': None,
            'labels': {},
            'descriptions': {},
        ...
        entity['labels'][lang] = {'language': lang, 'value': simplified['label']}
        if 'description' in simplified:
            entity['descriptions'][lang] = {'language': lang, 'value': simplified['description']}
        if 'aliases' in simplified:
            entity['aliases'][lang] = [{'language': lang, 'value': alias} for alias in simplified['aliases']]
        for stmt in simplified.get('statements', []):
            prop = stmt['claim']['property']
            value = stmt['claim']['value']
            references = [{'property': s['property'], 'value': s['value']} for s in stmt['references']] if 'references' in stmt else None
            qualifiers = [{'property': s['property'], 'value': s['value']} for s in stmt['claim']['qualifiers']] if 'qualifiers' in stmt['claim'] else None
            ...
            if qualifiers:
                ...
                for qualifier in qualifiers:
                    qprop = qualifier['property']
                    if qprop not in claim['qualifiers']:
                        claim['qualifiers'][qprop] = []
                        claim['qualifiers-order'].append(qprop)
                    claim['qualifiers'][qprop].append({
                        'snaktype': 'value',
                        'property': qprop,
                        'datavalue': qualifier['value']
                    })
            if references:
                claim['references'] = [{'snaks': {}, 'snaks-order': []}]
                claim_refs = claim['references'][0]['snaks']
                for ref in references:
                    rprop = ref['property']
                    if rprop not in claim_refs:
                        claim_refs[rprop] = []
                        claim['references'][0]['snaks-order'].append(rprop)
                claim_refs[rprop].append({
                    'snaktype': 'value',
                    'property': rprop,
                    'datavalue': ref['value']
                })
            if stmt['claim']['property'] not in entity['claims']:
                entity['claims'][stmt['claim']['property']] = []
            entity['claims'][stmt['claim']['property']].append(claim)
